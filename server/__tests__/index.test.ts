...
describe("server/index.ts (Copilot CLI preToolUse)", () => {
  function startCopilotServer(toolName: string, planContent?: string): Promise<RunningServer> {
    return new Promise((resolve, reject) => {
      const homeDir = mkdtempSync(join(tmpdir(), "plancop-copilot-test-"));

      // Write plan file if content provided
      if (planContent) {
        writeFileSync(join(homeDir, "plan-01.md"), planContent, "utf8");
      }
      ...
      child.once("exit", (code: number | null) => {
        if (!settled) {
          settled = true;
          // For fast-path tests (non-exit_plan_mode), resolve with exit info
          const server: RunningServer = {
            process: child,
            port: 0,
            token: "",
            ...
            getStdout: () => stdout,
            getStderr: () => stderr,
            waitForExit: () => Promise.resolve(code),
          };
          running.push(server);
          resolve(server);
        }
  ...
  it("exits immediately with code 0 for non-exit_plan_mode tool calls", async () => {
    const server = await startCopilotServer("bash");
    const code = await server.waitForExit();
    expect(code).toBe(0);
    // No server started, no stdout output needed (allow by default)
    expect(server.port).toBe(0);
  }, 15_000);

  it("exits with allow when exit_plan_mode but no plan file exists", async () => {
    const server = await startCopilotServer("exit_plan_mode");
    const code = await server.waitForExit();
    expect(code).toBe(0);
    const stdout = server.getStdout().trim();
    expect(stdout).toBeTruthy();
  ...
  it("serves GET /api/plan with copilot-cli origin", async () => {
    const server = await startCopilotServer("exit_plan_mode", "# Copilot Plan\n\nDo the thing");
    const response = await fetch(`http://127.0.0.1:${server.port}/api/plan`, {
      headers: { "Authorization": `Bearer ${server.token}` },
    });
    const body = await response.json();
  ...
  it("handles POST /api/approve and outputs permissionDecision allow", async () => {
    const server = await startCopilotServer("exit_plan_mode", "# Plan\n\nSteps here");
    const response = await fetch(`http://127.0.0.1:${server.port}/api/approve`, {
      method: "POST",
      headers: { "Authorization": `Bearer ${server.token}` },
    });
  ...
  it("handles POST /api/deny and outputs permissionDecision deny", async () => {
    const server = await startCopilotServer("exit_plan_mode", "# Plan\n\nSteps here");
    const response = await fetch(`http://127.0.0.1:${server.port}/api/deny`, {
      method: "POST",
      headers: { "Authorization": `Bearer ${server.token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ reason: "Rethink the approach" }),
    });
    ...
    expect(output).toEqual({
      ...
      permissionDecisionReason: "Rethink the approach",
    });
  });

  it("settles with deny on SIGTERM for Copilot flow", async () => {
    const server = await startCopilotServer("exit_plan_mode", "# Plan\n\nContent");
